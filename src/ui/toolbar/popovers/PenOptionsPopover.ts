...
/**
 * Floating popover for Pen tool options.
 * Serves ONLY the Pen tool — Highlighter has its own HighlighterOptionsPopover.
 */
export class PenOptionsPopover extends BasePopover {
    ...
    constructor(parent: HTMLElement, plugin: any, private toolbar: Toolbar, dismissBoundary?: HTMLElement) {
        super(parent, plugin, 'ink-pen-options-popover', dismissBoundary);
    }
    ...
    protected buildContent(): void {
        ...
        this.thicknessSlider = thicknessInputRow.createEl('input', {
            ...
            attr: { type: 'range', min: '1', max: '30', step: '1', value: '4' }
        }) as HTMLInputElement;
        this.thicknessValSpan = thicknessInputRow.createSpan({ cls: 'ink-popover-size-val', text: '4px' });
        ...
        const updateThickness = (e?: Event) => {
            ...
            if (this.activeEngine) {
                if (typeof this.activeEngine.setToolSize === 'function') {
                    this.activeEngine.setToolSize('pen', val);
                } else if (typeof (this.activeEngine as any).setPenSize === 'function') {
                    (this.activeEngine as any).setPenSize(val);
                }
                (this.activeEngine as any).activeProfileId = '';
                if (this.toolbar.plugin?.settings) {
                    this.toolbar.plugin.settings.activePenProfileId = null;
                }
        ...
        this.smoothnessSlider = smoothnessInputRow.createEl('input', {
            ...
            attr: { type: 'range', min: '0.0', max: '1.0', step: '0.05', value: '0.25' }
        }) as HTMLInputElement;
        this.smoothnessValSpan = smoothnessInputRow.createSpan({ cls: 'ink-popover-size-val', text: '0.25' });
        ...
        const updateSmoothness = (e?: Event) => {
            ...
            if (this.isSyncing) return;
            if (this.activeEngine) {
                this.activeEngine.setPenSmoothing?.(val);
                (this.activeEngine as any).activeProfileId = '';
                if (this.toolbar.plugin?.settings) {
                    this.toolbar.plugin.settings.activePenProfileId = null;
                }
    ...
    /**
     * Opens and populates the pen options popover anchored to the given button.
     */
    public showPenOptions(anchorBtn: HTMLElement, engine: InkEngine): void {
    ...
    private populateContent(engine: InkEngine): void {
        ...
        try {
            ...
            const activeProfileId = rawProfileId !== undefined ? rawProfileId : 'pen-rounded';
            const thickness = typeof engine.getToolSize === 'function' ? engine.getToolSize('pen') : 4;
            const smoothing = typeof engine.getPenSmoothing === 'function' ? engine.getPenSmoothing() : 0.25;

            // 1. Profile Carousel
            if (this.profileCarouselEl) {
                this.profileCarouselEl.empty();
                const profiles = PenProfileRegistry.getAll().filter(p => p.toolType === 'pen');
                ...
                profiles.forEach((profile) => {
                    ...
                    pill.addEventListener('click', (e) => {
                        ...
                        try {
                            ...
                            if (this.toolbar.plugin?.settings) {
                                this.toolbar.plugin.settings.activePenProfileId = profile.id;
                            }
                            if (typeof engine.setToolSize === 'function') {
                                engine.setToolSize('pen', profile.baseWidth);
                            }
    ...
    private syncStarState(engine: InkEngine): void {
        ...
        const toolType = 'pen';
        const profileId = (engine as any).activeProfileId || 'pen-rounded';
        const width = typeof engine.getToolSize === 'function' ? engine.getToolSize('pen') : 4;
        const currentColor = (engine as any).toolContext?.currentColor ?? '#1a1a1a';
    ...
    private handleStarToggle(): void {
        ...
        const currentProfileId = (engine as any).activeProfileId || 'pen-rounded';
        const strokeWidth = engine.getToolSize('pen') ?? 4;
        const smoothingLevel = engine.getPenSmoothing?.() ?? 0.25;
        ...
        const color = serializeColor(parsed.rgb, parsed.alpha, false);
        ...
        if (existingIdx !== -1) {
            ...
            new Notice('Removed pen preset from Pencil Case.');
        } else {
            ...
            const defaultName = registryProfile?.name || 'Pen';
            ...
            new Notice('Saved pen preset to Pencil Case!');
        }
